...
def login(request):
	if(request.method == "POST"):
		...
		cursor.execute ("select password,id from jts_employees where userName = '%s'" %(username))
		...
		if(len(rows)>0):
			for row in rows:
				...
				if (verification == True):
					cursor = connection.cursor()
					cursor.execute("insert into session_history(loginTime,userId) values(CURRENT_TIMESTAMP(),'%s')" % (userid))
					cursor.close()
...
def add_product(request):
	if(request.method == "POST"):
		...
		if((data1.get('productno') is None)):
		...
		if((data1.get('productmodel') is None)):
		...
		if((data1.get('productcompany') is None)):
		...
		image_result = open('/var/www/html/InventoryApp/images/%s.jpg' %(productname), 'wb')   
		image_result.write(image_64_decode)
		#checks db	
		is_history_rec_needed = True'''
		
		warehouse_status_get = inv_products.objects.filter(productName=productname)
		if(len(warehouse_status_get) > 0): #just update the ChangeDate         
			is_history_rec_needed = False
			output = '{"error_code":"4", "error_desc": "Response= Your product already exists."}'
			logging.debug("add_product:"+ output)
			return HttpResponse(output)
...
def update_product(request):
	if(request.method == "POST"):
		...
		if((data1.get('productdescription') is None)):
			output_str += ",productdescription is mandatory"
			output = '{"error_code":"2", "error_desc": "%s"}' %output_str
			logging.debug("productdescription:"+ output)
			return HttpResponse(output)
...
#####################################################################################################################################
def get_productposition(request):
	if(request.method == "POST"):
		logging.debug("product_position:request is from ip: %s" %request.META.get('REMOTE_ADDE'))
		output_str = "Listing products based on position.."
		try:
			data1 = json.loads((request.body).decode('utf-8'))
			print request.body
			#return data1
			...
			output_str += ",invalid input, no proper JSON request "
			output = '{"error_code":"2", "error_desc": "%s"}' %(output_str )
			logging.debug("product_position:"+ output)
			return HttpResponse(output)
			#return "value error"
		...
		if((data1.get('positionid') is None) or ((data1.get('positionid') is not None) and (len(data1['positionid']) <=0))):
			output_str += ",positionid is required"
			output = '{"error_code":"2", "error_desc": "%s"}' %output_str
			logging.debug("positionid:"+ output)
			return HttpResponse(output)
		...
		try:
			cursor = connection.cursor()
			cursor.execute("select q.positionId_id,p.Position,q.productId_id,pr.productName,q.quantityId,q.quantity from inv_quantity q JOIN inv_position p ON q.positionId_id = p.id JOIN inv_products pr ON q.productId_id = pr.id where q.PositionId_id = '%s'"%(positionid))
			rows = cursor.fetchall()
			cursor.close()
			if(len(rows) > 0):
				objects_list = []
				for row in rows:
					...
					d['quantity'] = row[5]
					objects_list.append(d)
				json_output='{"product_position":'	
				json_output+= json.dumps(objects_list,indent = 3)
				json_output+='}'
				return HttpResponse(json_output)
			else:
				output = '{"error_code":"2", "error_desc": "product dosent exists in this position enter a product"}'
				logging.debug("product_position:"+ output)
				return HttpResponse(output) 
		except Exception, e:
			err_desc = 'product_position:exception details:[%s],[%s]' %((sys.exc_info()[0]), (sys.exc_info()[1]))
			logging.debug("product_position:"+ err_desc)
			output = '{"error_code":"2", "error_desc": "Please enter a valid position"}' 
			logging.debug("product_position:"+ output)
			return HttpResponse(output)
	else:
		logging.debug("product_position: request is from the IP:%s" %request.META.get('REMOTE_ADDR'))
		output = '{"error_code":"2", "error_desc": "GET is not supported"}' 
		logging.debug("product_position:"+ output)
		return HttpResponse(output)
#############################################################################################################################################
...
def update_requisitiondetails(request):
	if(request.method == "POST"):
		...
		return HttpResponse(output)
##################################################################################################################################
def get_purchasableproducts(request):
	cursor = connection.cursor()
	cursor.execute("select q.productId_id,p.productName,po.Position,q.quantity,p.reorderlevel from inv_quantity as q JOIN inv_products p ON q.productId_id = p.id Join inv_position po ON q.positionId_id = po.id WHERE q.quantity <= p.reorderlevel")
	rows = cursor.fetchall()
	cursor.close()
	objects_list = []
	for row in rows:
		...
		d['productid']=row[0]
		d['productname'] = row[1]
		d['productposition'] = row[2]
		d['reorderlevel'] = row[4]
		d['currentquantity'] = row[3]
		objects_list.append(d)
	json_output='{"purchasable_product_details":'	
	json_output+= json.dumps(objects_list,indent = 3)
	json_output+='}'
	return HttpResponse(json_output)
...
def get_sessionid(request):
	if(request.method == "POST"):
		...
		try:
			cursor = connection.cursor()
			cursor.execute("select sessionId from session_history where userId = '%s' order by sessionId desc limit 1" %(userid))
			rows = cursor.fetchall()
			cursor.close()
			objects_list = []
			for row in rows:
			...
			err_desc = 'get_sessionid:exception details:[%s],[%s]' %((sys.exc_info()[0]), (sys.exc_info()[1]))
			logging.debug("get_sessionid:"+ err_desc)
			output = '{"error_code":"2", "error_desc": "Invalid username"}' 
			logging.debug("get_user:"+ output)
			return HttpResponse(err_desc)
	else:
		logging.debug("get_sessionid: request is from the IP:%s" %request.META.get('REMOTE_ADDR'))
		output = '{"error_code":"2", "error_desc": "GET is not supported"}' 
		logging.debug("get_sessionid:"+ output)
		return HttpResponse(output)

##############################################################################################################################
def add_purchaserequision(request):
	...
	if(request.method == "POST"):
		...
		if((data1.get('sessionid') is None) or ((data1.get('sessionid') is not None) and (len(data1['sessionid']) <=0))):
			output_str += ",sessionid is mandatory"
			output = '{"error_code":"2", "error_desc": "%s"}' %output_str
			logging.debug("sessionid:"+ output)
			return HttpResponse(output)
		...
		try:
			...
			cursor = connection.cursor()
			cursor.execute("insert into purchase_requisition(requestedDate,dueDate,sessionId,statusId) values(CURRENT_TIMESTAMP(),'%s','%s','%s')" %(duedate,sessionid,status))
			cursor.close()
			time.sleep(0.200)
			
			cursor = connection.cursor()
			cursor.execute("select orderId from purchase_requisition where sessionId = '%s' order by orderID desc limit 1" %(sessionid))
			rows = cursor.fetchall()
			cursor.close()
			if len(rows) > 0:
				for row in rows:
					...
					time.sleep(0.100)
					for rec in product:
						index1=product.index(rec)
						product1 = rec
						quantity1 = qty[index1]
						#print product1
						#print quantity1
						cursor = connection.cursor()
						cursor.execute("insert into purchase_details(quantityrequested,productId,purchaseorderId,statusId) values('%s','%s','%s','%s')" %(quantity1,product1,orderid,status))
						cursor.close()
						time.sleep(0.100)
			output = '{"error_code":"1", "error_desc": "purchase order submitted succesfully"}' 
			logging.debug("add_purchaserequisition:"+ output)
			return HttpResponse(output)	
			
		except Exception, e:
			err_desc = 'add_purchaserequisition:exception details:[%s],[%s]' %((sys.exc_info()[0]), (sys.exc_info()[1]))
			logging.debug("add_purchaserequisition:"+ err_desc)
			output = '{"error_code":"2", "error_desc": "problem placing the purchase order"}' 
			logging.debug("add_purchaserequisition:"+ output)
			return HttpResponse(err_desc)
	else:
		logging.debug("add_purchaserequisition: request is from the IP:%s" %request.META.get('REMOTE_ADDR'))
		output = '{"error_code":"2", "error_desc": "GET is not supported"}' 
		logging.debug("add_purchaserequisition:"+ output)
		return HttpResponse(output)
		
#####################################################################################################################################
def get_purchaserequisitions(request):
	cursor = connection.cursor()
	cursor.execute("select pr.*,s.statusName from purchase_requisition pr JOIN inv_status s ON pr.statusId = s.id")
	rows = cursor.fetchall()
	cursor.close()
	objects_list = []
	for row in rows:
...
def get_purchasedetails(request):
	if(request.method == "POST"):
		logging.debug("search_employees:request is from ip: %s" %request.META.get('REMOTE_ADDE'))
		output_str = "getting purchasedetails.."
		try:
			data1 = json.loads((request.body).decode('utf-8'))
			print request.body
			#return data1
			...
			output_str += ",invalid input, no proper JSON request "
			output = '{"error_code":"2", "error_desc": "%s"}' %(output_str )
			logging.debug("purchase_details:"+ output)
			return HttpResponse(output)
			#return "value error"
		...
		if((data1.get('orderid') is None) or ((data1.get('orderid') is not None) and (len(data1['orderid']) <=0))):
			output_str += ",orderid is required"
			output = '{"error_code":"2", "error_desc": "%s"}' %output_str
			logging.debug("orderid:"+ output)
			return HttpResponse(output)
		...
		try:
			cursor = connection.cursor()
			cursor.execute("select p.id,p.purchaseorderId,p.productId,pr.productName,pr.productCompany,pr.productDescription,p.quantityrequested,p.quantityreceived,p.statusId,s.statusName from purchase_details p JOIN inv_products pr ON p.productId = pr.id JOIN inv_status s ON p.statusId = s.id WHERE p.purchaseorderId = '%s'" %(orderid))
			rows = cursor.fetchall()
			cursor.close()
			objects_list = []
			for row in rows:
				d = collections.OrderedDict()
				d['purchasedetailsid']=row[0]
				d['purchaseorderid'] = row[1]
				d['productid'] = row[2]
				d['productname'] = row[3]
				d['productcomapany'] = row[4]
				d['productdescription'] = row[5]
				d['quantityrequested'] = row[6]
				d['quantityreceived'] = row[7]
				d['status'] = row[9]
				objects_list.append(d)
			json_output='{"purchase_details":'	
			json_output+= json.dumps(objects_list,indent = 3,sort_keys = True, default = str)
			json_output+='}'
			logging.debug("purchase_details:")
			return HttpResponse(json_output)
		except Exception, e:
			err_desc = 'purchase_details:exception details:[%s],[%s]' %((sys.exc_info()[0]), (sys.exc_info()[1]))
			logging.debug("purchase_details:"+ err_desc)
			output = '{"error_code":"2", "error_desc": "invalid orderid"}' 
			logging.debug("purchase_details:"+ output)
			return HttpResponse(err_desc)
	else:
		logging.debug("purchase_details: request is from the IP:%s" %request.META.get('REMOTE_ADDR'))
		output = '{"error_code":"2", "error_desc": "GET is not supported"}' 
		logging.debug("purchase_details:"+ output)
		return HttpResponse(output)
		
###################################################################################################################################
def update_purchaserequisition(request):
	if(request.method == "POST"):
		logging.debug("update_purchase_requisition_details:request is from ip: %s" %request.META.get('REMOTE_ADDE'))
		output_str = "updating purchase requisition details.."
		try:
			data1 = json.loads((request.body).decode('utf-8'))
			print request.body
			#return data1
			...
			output_str += ",invalid input, no proper JSON request "
			output = '{"error_code":"2", "error_desc": "%s"}' %(output_str )
			logging.debug("update_purchase_requisition_details:"+ output)
			return HttpResponse(output)
			#return "value error"
		...
		if((data1.get('purchasedetailsid') is None) or ((data1.get('purchasedetailsid') is not None) and (len(data1['purchasedetailsid']) <=0))):
			output_str += ",purchasedetailsid is mandatory"
			output = '{"error_code":"2", "error_desc": "%s"}' %output_str
			logging.debug("purchasedetailsid:"+ output)
			return HttpResponse(output)
		
		else:
			purchasedetailsid = data1['purchasedetailsid']
			
		if((data1.get('quantityreceived') is None) or ((data1.get('quantityreceived') is not None) and (len(data1['quantityreceived']) <=0))):
			output_str += ",quantityreceived is mandatory"
			output = '{"error_code":"2", "error_desc": "%s"}' %output_str
			logging.debug("quantityreceived:"+ output)
			return HttpResponse(output)
		
		else:
			quantityreceived = data1['quantityreceived']
		...
		try:
			...
			receivedquantity =[]
			statusid = []
			sdetailsid = str(purchasedetailsid)[1:-1]
			sreceivedquantity = str(quantityreceived)[1:-1]
			sstatusid = str(status)[1:-1]
			
			for details in sdetailsid.split(","):
				detailsid.append(details)
			print detailsid
			
			for quant in sreceivedquantity.split(","):
				receivedquantity.append(quant)
			print receivedquantity
			
			for stat in sstatusid.split(","):
				statusid.append(stat)
			print statusid
			
			for rec in detailsid:
				index1 = detailsid.index(rec)
				details1 = rec
				quantity1 = receivedquantity[index1]
				status1 = statusid[index1]
				#print details1
				#print quantity1
				#print status1
				cursor = connection.cursor()
				cursor.execute("update purchase_details SET quantityreceived = '%s',statusId = '%s' where id = %s" %(quantity1,status1,details1))
				cursor.close()
				time.sleep(0.100)
			output = '{"error_code":"1", "error_desc": "purchase details updated"}' 
			logging.debug("update_requisition_details:"+ output)
			return HttpResponse(output)	
			...
			err_desc = 'purchase_details:exception details:[%s],[%s]' %((sys.exc_info()[0]), (sys.exc_info()[1]))
			logging.debug("purchase_details:"+ err_desc)
			output = '{"error_code":"2", "error_desc": "invalid details"}' 
			logging.debug("purchase_details:"+ output)
			return HttpResponse(err_desc)
	else:
		logging.debug("purchase_details: request is from the IP:%s" %request.META.get('REMOTE_ADDR'))
...
####################################################################################################################################
def update_purchasestatus(request):
	if(request.method == "POST"):
		logging.debug("update_purchase_requisiton_status:request is from ip: %s" %request.META.get('REMOTE_ADDE'))
		output_str = "updating purchase requisition status.."
		try:
			data1 = json.loads((request.body).decode('utf-8'))
			print request.body
			#return data1
			...
			output_str += ",invalid input, no proper JSON request "
			output = '{"error_code":"2", "error_desc": "%s"}' %(output_str )
			logging.debug("update_purchase_requisiton_status:"+ output)
			return HttpResponse(output)
			#return "value error"
		...
		if((data1.get('orderid') is None) or ((data1.get('orderid') is not None) and (len(data1['orderid']) <=0))):
			output_str += ",orderid is mandatory"
			output = '{"error_code":"2", "error_desc": "%s"}' %output_str
			logging.debug("orderid:"+ output)
			return HttpResponse(output)
		...
		try:
			purchase_rec = purchase_requisition.objects.filter(orderId = orderid).update(status = statusid)
			output = '{"error_code":"1", "error_desc": "status updated"}' 
			logging.debug("update_purchase_requisiton_status:"+ output)
			return HttpResponse(output)
			
		except Exception, e:
			err_desc = 'update_purchase_requisiton_status:exception details:[%s],[%s]' %((sys.exc_info()[0]), (sys.exc_info()[1]))
			logging.debug("update_purchase_requisiton_status:"+ err_desc)
			output = '{"error_code":"2", "error_desc": "Failed to update the record/ID doesnot exists"}' 
			logging.debug("update_purchase_requisiton_status:"+ output)
			return HttpResponse(output)
	else:
		logging.debug("update_purchase_requisiton_status: request is from the IP:%s" %request.META.get('REMOTE_ADDR'))
		output = '{"error_code":"2", "error_desc": "GET is not supported"}' 
		logging.debug("update_purchase_requisiton_status:"+ output)
		return HttpResponse(output)
