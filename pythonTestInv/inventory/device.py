...
class addposition:
         def POST(self):
                ...
                data1 = json.loads(data)
                print data
		sql = "insert into inv_position(positionId,warehouseId,position,statusId) values('%s','%s','%s','%s')" %(positionid,warehouseid,position,statusid)
		try:
			cursor.execute(sql)
			db.commit()
			return status1
...
class addwarehouse:
         def POST(self):
                ...
                data1 = json.loads(data)
                print data
		sql = "insert into inv_warehouse(warehouseId,warehouseName,locationId,statusId) values ('%s','%s','%s','%s')" %(warehouseid,warehousename,locationid,statusid)
		try:
			cursor.execute(sql)
			db.commit()
			return status1
